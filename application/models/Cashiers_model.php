...
class Cashiers_model extends CI_Model
{
    ...
    public function update_queue_complete_status($id, $status)
    {
        ...
        // If the status is "served", set the end_service_time to the current timestamp
        if ($status === 'served') {
            $this->db->set('end_service_time', 'NOW()', FALSE); // Use MySQL's NOW() function to set the current timestamp
        }

        // Try updating the priority_queue first
        $this->db->where('id', $id);
        $this->db->update('priority_queue');
        $affected_rows_priority = $this->db->affected_rows();

        // If no rows were affected, try updating the regular_queue
    ...
    public function update_status_and_time($customerId, $queue_number)
    {
        ...
        $this->db->set('end_service_time', 'NOW()', false); // Use SQL NOW() function for current timestamp
    ...
    {
        // Update the notify_queue in priority_queue
        $this->db->set('notify_queue', $show_on_monitor);
        $this->db->where('id', $id);
        $this->db->update('priority_queue');
        $affected_rows_priority = $this->db->affected_rows();

        // If no rows were affected in priority_queue, try updating regular_queue
        if ($affected_rows_priority == 0) {
            $this->db->set('notify_queue', $show_on_monitor);
            $this->db->where('id', $id);
            $this->db->update('regular_queue');
    ...
    public function fetch_pending_queues()
    {
        ...
        // Fetch pending queue numbers from priority_queue
        ...
        // Fetch pending queue numbers from regular_queue
        $this->db->select('queue_number');
    ...
    {
        // Sanitize and validate the Cashier ID
    ...
    {
        //fetching from the priority queue
        ...
        //fetching from the regular queue
        $this->db->select('id, name, queue_number, student_number, service_type, payment_for, payment_mode, start_service_time, end_service_time');
        $this->db->where('id', $id);
